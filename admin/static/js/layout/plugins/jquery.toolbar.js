		...
		var addControlls=function(td,items){
			...
			for (var i = 1; i < items.length; i++) {
				...
				if (btn == "-") {
					...
					if(btn.type=='button' || btn.text=='搜索'){
						var b=$('<button type="button" class="button button-primary">'+btn.text+'</button>').appendTo(td);
						b[0].onclick = eval(btn.handler || function () {});
					}else if(btn.type==undefined){
						...
						td.append(label);
						var b = $('<select name="'+btn.name+'"  id="'+btn.name+'" style="min-width:80px;height:30px;" '+(btn.change?'change='+btn.change:'')+'></select>').appendTo(td);
						...
						td.append(label);
						var b = $('<input name="'+btn.name+'" id="'+btn.name+'" style="height:30px;">').appendTo(td);
						...
						td.append(label);
						var b = $('<input name="'+btn.name+'" value="'+(btn.value?btn.value:'')+'" style="margin:0px;padding:0px;width:'+btn.width+'px;border:1px solid #cecece;height:30px;">').appendTo(td);
						if(btn.id!=undefined) b.attr('id',btn.id);
						b.blur(function(){
