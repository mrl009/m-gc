<div style="padding:5px" class="listLottery" id="odds_s">
    <div style="padding-bottom: 10px">
        <button type="button" class="btn btn-success" onclick="OddsSs.updateOdds()">賠率初始化</button>
        <button type="button" class="btn btn-warning" onclick="OddsSs.delCache('play')">清除所有緩存</button>
        <button type="button" class="btn btn-info delone" onclick="OddsSs.delCache('e')">清除緩存</button>
        <span class="label label-danger">賠率/退水修改後請壹定要清除緩存,否則無法下註</span>
    </div>
    <div id="cpLists_s">
        <?php foreach($games as $v){?>
            <a type="button" class="btn btn-link cpbtn" tmp="<?php echo $v['tmp']?>" val="<?php echo $v['id']?>" onclick="OddsSs.change(this)"><?php echo $v['name']?></a>
        <?php }?>
    </div>
    <div style="width: 100%;" id="newsOdds_s"></div>
</div>
<script>
    var OddsSs={
        init:function(){
            var id = $('#odds_s .cpbtn').eq(0).attr('val');
            var tmp = $('#odds_s .cpbtn').eq(0).attr('tmp');
            OddsSs.getlist(id,tmp);
            $('#odds_s .delone').text('清除'+$('#odds_s .cpbtn').eq(0).text()+'緩存');
            $('#odds_s .cpbtn').eq(0).removeClass('btn-link');
            $('#odds_s .cpbtn').eq(0).addClass('btn-danger');
        },
        getlist:function(id,tmp){
            layer.msg('加載中', {icon: 16,shade: 0.01});
            $.get('odds/get_list_new?id='+id+'&tmp='+tmp+'&ctg=<?php echo $ctg?>',function(c){
                $('#newsOdds_s').html(c);
                console.log($('#newsOdds_s'))
                layer.closeAll();
            });
        },
        change:function(e){
            $('#odds_s .delone').text('清除'+$(e).text()+'緩存');
            $('#odds_s .cpbtn').removeClass('btn-danger');
            $('#odds_s .cpbtn').addClass('btn-link');
            $(e).removeClass('btn-link');
            $(e).addClass('btn-danger');
            OddsSs.getlist($(e).attr('val'),$(e).attr('tmp'));
        },
        saveOdds:function(e){
        ...
        updateOdds:function(){
            var id=$('#cpLists_s .btn-danger').attr('val');
            var tmp=$('#cpLists_s .btn-danger').attr('tmp');
        ...
        delCache:function(e){
            var id=$('#cpLists_s .btn-danger').attr('val');
            var tmp=$('#cpLists_s .btn-danger').attr('tmp');
            var title=$('#cpLists_s .btn-danger').text();
            if(e=='play') {
                id='';
                title='所有彩票';
            }
