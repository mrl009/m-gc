...
<div class="easyui-tabs" data-options="tabHeight:45,fit:true,tabPosition:'top',border:false,pill:true,narrow:true,justified:true" >
    <div title="彩種列表" style="padding:10px;">
        ...
        <script>
            $("#<?php echo $id;?>").DataSource({url:'game/get_list?ctg=sc',
                fields:[[
                    ...
                    {field:'op',    title:'操作(拖动列表可以排序)', width:150,align:'center',sortable:false,formatter:function(v,r){
                        return '<a herf="#"  onclick=updateScGameStatus('+r.id+','+r.open+')>'+(r.open==1?'<span class="label label-success">下架</span>':'<span class="label label-danger">上架</span>')+'</a>&nbsp;&nbsp;' +
                            '<a herf="#"  onclick=updateScGameHome('+r.id+','+r.home+','+r.open+')>'+(r.home==1?'<span class="label label-success">下首页</span>':'<span class="label label-danger">上首页</span>')+'</a>';
                    }}
                ]],
                tools:[
                    {instant:false}
                ...
                onDrop:function (t, s) {
                    ...
                    if (open) {
                        ...
                        if (r.length) {
                            ...
                            $.get(WEB+'game/ajax_get_cp',{ctg:'gc'},function(json){
            ...
            function updateScGameStatus(id,status){
            ...
            function updateScGameHome(id,status,open){
                if (open==0) {
                    Core.error('請先上架該彩種');
                    return false;
                }
    ...
    <div title="首頁彩種排序" style="padding:10px;">
        ...
        <script>
            $("#<?php echo $id2;?>").DataSource({url:'game/get_home_list?ctg=sc',
                fields:[[
                    {field:'id',    title:'ID ',width:40, sortable:false},
                    {field:'name',  title:'彩种名字',width:100,align:'center'},
                    {field:'tname', title:'展示分类',width:100,align:'center'},
            ...
            $('#<?php echo $id2?>').datagrid({
                onDrop:function (t, s) {
                    ...
                    if (r.length > 1) {
                        ...
                        $.get(WEB+'game/ajax_get_cp_index',{ctg:'gc'},function(json){
                            var c = JSON.parse(json);
                            if (c.status == 'OK') {
